...
class TestMixtureDistribution(object):
    ...
    @pytest.mark.parametrize("dtype, mixing_prob, mixing_prob_isSamples, rv, rv_isSamples, var, var_isSamples, num_samples, one_hot_encoding, normalization", [
        (np.float64, np.random.rand(5,3)+1e-2, True, np.random.rand(5,3)+1e-2, True, np.random.rand(5,3,3)+1e-2, True, 5, True, True),
        (np.float64, np.random.rand(3)+1e-2, False, np.random.rand(5,3)+1e-2, True, np.random.rand(5,3,3)+1e-2, True, 5, True, False),
        ])
